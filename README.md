...
- 窗口行政動態索引 MVP 已加入專用 SQLite、年份/關鍵字搜尋 API、既有靜態頁動態注入，以及 Word/PDF/PSD 等副檔名卡片與開啟端點。
- 後台已加入「專案/活動」欄位與照片/行政欄位連動；手動「重新整理現有前台資料」可在背景重建照片與窗口索引，失敗時保留上一個可用索引。
- H 實機完整照片索引掃描 464,251 筆，照片階段約 115.6 秒、整體約 132.4 秒、錯誤 0。測試當下已上架窗口根目錄沒有檔案，因此窗口索引為 0 筆，仍需人工上架 Office/PDF/PSD 驗收。
- runtime feature commits：`51c6a0f`、`869105b`、`aa9a14b`、`e785f38`。
- 窗口行政動態索引已完成機械驗證，但 Office/PSD 真實開啟與窗口頁人工操作仍待驗收；整套後台尚不可宣稱正式完成。
